...
public:
  ...
  int get_layersize(){
      return graph->gnnctx->layer_size.size() - 1;
  } 
  ...
  void gather_neibor_v(int i, std::string manner){
    graph->rtminfo->curr_layer = i;
    if (i != 0) {
      X[i] = drpmodel(X[i]);
    }
    if(manner == std::string("sum")){
       gt->PropagateForwardCPU_Lockfree_multisockets(X[i], Y[i], subgraphs);
    }
    cp->op_push(X[i], Y[i], nts::autodiff::DIST_CPU);
  ...
  void py_vertexForward(int layer, std::string jihuo, std::string cul){
    NtsVar y;
    int i = graph->rtminfo->curr_layer;
    if(layer < graph->gnnctx->layer_size.size()-1){
        if(jihuo == std::string("relu")){
          if(cul == std::string("matmul")){
            y = torch::relu(P[i]->forward(Y[i])).set_requires_grad(true);
          }
        }
    }
    else if(layer == graph->gnnctx->layer_size.size()-1){
       y = P[i]->forward(Y[i]);
       y = y.log_softmax(1);
    }
    cp->op_push(Y[i], y, nts::autodiff::NNOP);
    X[i+1] = y;
  }

  void clear_gradient(int i_i)
  {
    if (i_i != 0) {
